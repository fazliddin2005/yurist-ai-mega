...
router.post('/google', async (req, res) => {
  try {
    ...
    if (!user) {
      ...
      // Mavjud foydalanuvchi - Google bilan keling, ma'lumotlarni yangilash
      let changed = false;
      if (!user.googleId) { user.googleId = googleId; changed = true; }
      if (picture && !user.avatar) { user.avatar = picture; changed = true; }
      // Google orqali kirgan bo'lsa, email tasdiqlangan hisoblanadi
      if (!user.emailVerified && email) { user.emailVerified = true; changed = true; }

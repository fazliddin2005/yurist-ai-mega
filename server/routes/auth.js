...
router.post('/login', authLimiter, async (req, res) => {
  try {
    ...
    if (!valid) {
      // Muvaffaqiyatsiz login urinishini hisoblaymiz (brute-force himoyasi)
      user.failedLoginCount = (user.failedLoginCount || 0) + 1;
      if (user.failedLoginCount >= 10) {
        user.lockedUntil = new Date(Date.now() + 30 * 60 * 1000); // 30 daqiqa lock
      }
      await user.save();
      return res.status(401).json({ error: "Email/telefon yoki parol noto'g'ri" });
    }

    // Account lock tekshirish
    if (user.lockedUntil && user.lockedUntil > new Date()) {
      const minutesLeft = Math.ceil((user.lockedUntil - Date.now()) / 60000);
      return res.status(423).json({ error: `Akkount vaqtincha bloklangan. ${minutesLeft} daqiqadan keyin qaytadan urinib ko'ring.`, code: 'ACCOUNT_LOCKED' });
    }

    // Muvaffaqiyatli kirish -- failed count ni tozalaymiz
    ...
    // 2FA tekshirish: yoqilgan bo'lsa to'liq token bermay, 2FA kodi talab qilamiz
    if (user.twoFactorEnabled) {
      await user.save();
      logActivity({ type: '2fa_challenge', userId: user.id, meta: { ip: req.ip } });
      return res.json({
        requires2FA: true,
        userId: user.id, // frontend /api/auth/2fa/verify ga yuboradi
        message: "Authenticator ilovasidagi 6 xonali kodni kiriting",
      });
    }
...
router.post('/refresh', async (req, res) => {
  try {
    ...
    const { verifyRefreshToken, createAccessToken, hashToken } = require('../services/tokenService');
    const payload = verifyRefreshToken(refreshToken);
    if (!payload) return res.status(401).json({ error: 'Refresh token yaroqsiz yoki muddati tugagan', code: 'REFRESH_EXPIRED' });

    const user = await User.findById(payload.uid);
    if (!user) return res.status(401).json({ error: 'Foydalanuvchi topilmadi' });

    // DB da saqlangan refresh token lardan birimi?
    const hash = hashToken(refreshToken);
    if (!(user.refreshTokens || []).includes(hash)) {
      // Token DB da yo'q -- token rotation yoki o'g'irlanish bo'lishi mumkin
      // Xavfsiz: barcha refresh tokenlarni o'chiramiz
      user.refreshTokens = [];
      await user.save();
      return res.status(401).json({ error: 'Refresh token bekor qilingan. Qaytadan kiring.', code: 'REFRESH_REVOKED' });
    }

    const newAccessToken = createAccessToken(user.id);
    res.json({ accessToken: newAccessToken, expiresIn: 900 }); // 15 daqiqa = 900 soniya
  } catch (e) {
    console.error('[auth/refresh]', e.message);
    res.status(500).json({ error: 'Token yangilashda xato' });
  }
});
...
router.post('/logout/all', requireAuth, async (req, res) => {
  ...
  await req.user.save();
  logActivity({ type: 'logout_all_devices', userId: req.user.id, meta: { ip: req.ip } });
  res.json({ ok: true, message: "Barcha qurilmalardan chiqildi" });
});

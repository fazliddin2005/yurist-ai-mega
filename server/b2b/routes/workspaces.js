...
// PUT /api/b2b/workspaces/:workspaceId/branding -- white-label sozlash
router.put('/:workspaceId/branding', ws.requireWorkspaceAccess('admin'), async (req, res) => {
  try {
    const { logo, primaryColor, companyName, domain, favicon, welcomeMessage, footerText } = req.body;
    ...
    res.json({ ok: true, branding: workspace.branding });
  } catch (e) {
    console.error('[workspace/branding]', e.message);
    res.status(500).json({ error: "Branding sozlashda xato" });
  }
});

// GET /api/b2b/workspaces/:workspaceId/branding -- public branding (login sahifasi uchun)
router.get('/:workspaceId/branding', async (req, res) => {
  try {
    ...
    // Faqat public branding ma'lumotlari
    res.json({
      branding: workspace.branding || {},
      name: workspace.name,
    });
  } catch (e) {
    res.status(500).json({ error: "Branding ma'lumotini olishda xato" });
  }
});

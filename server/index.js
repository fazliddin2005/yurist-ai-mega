...
const clauseAnalysisRouter = require('./routes/clauseAnalysis');
const matterRouter = require('./routes/matter'); // Google va Apple login
...
// CSP o'chirilgan: index.html da yuzlab onclick/oninput atributlar bor,
// ular nonce bilan ishlamaydi. XSS himoyasi input sanitization orqali qilingan.
app.use(helmet({
...
const allowedOrigins = process.env.ALLOWED_ORIGINS
  ...
  : null;
// CORS: ALLOWED_ORIGINS sozlanmagan bo'lsa ham, wildcard '*' emas
// balki ma'lum xavfsiz originlarga ruxsat beramiz
const defaultOrigins = [
  'http://localhost:3000',
  'http://localhost:3001',
  'https://yurist-ai-mega.vercel.app',
];
const corsOrigins = allowedOrigins || defaultOrigins;
app.use(cors({
  origin: (origin, cb) => {
...
// JSON body limit: fayl yuklash (base64) yo'llari uchun 5mb, odatiy yo'llar uchun 100kb
// Bu DoS hujumlaridan himoya qiladi (katta body = katta parsing xarajati)
app.use((req, res, next) => {
  // /chat/file, /risk, /audit -- base64 fayl yuboradilar
  const isFileRoute = /\/(chat\/file|risk|audit)/.test(req.path);
  return express.json({ limit: isFileRoute ? '5mb' : '100kb' })(req, res, next);
});
...
app.get('/api/health', async (req, res) => {
  ...
  const ok = dbStatus === 'ok' || dbStatus === 'unknown';
...
// /api/auth/config -- MongoDB siz ishlashi SHART (frontend Google login uchun
// kerak, DB ulanishidan oldin chaqiriladi). MongoDB middleware dan OLDIN!
...
const { globalApiLimiter, suspiciousRequestDetector, responseSizeLimiter, extraSecurityHeaders } = require('./middleware/security');
app.use('/api', globalApiLimiter);
app.use('/api', suspiciousRequestDetector);
app.use('/api', responseSizeLimiter);
app.use(extraSecurityHeaders); // X-Frame-Options, Referrer-Policy, Permissions-Policy

// ---- HTTP CACHE HEADERS ----
// Statik va sekin o'zgaradigan API javoblari uchun cache sarlavhalari.
// Bu brauzer va Vercel Edge Network'ni ishlatadi -- DB yukini kamaytiradi.
app.use('/api/catalog', (req, res, next) => {
  // Qonunlar katalogi -- kuniga 1 marta o'zgaradi
  res.setHeader('Cache-Control', 'public, max-age=3600, stale-while-revalidate=86400');
  next();
});
app.use('/api/auth/config', (req, res, next) => {
  // Config -- 5 daqiqa cache
  res.setHeader('Cache-Control', 'public, max-age=300');
  next();
});
// Private/authenticated endpointlar -- hech qachon cache qilinmaydi
['/api/chat', '/api/documents', '/api/cases', '/api/risk', '/api/tts'].forEach(path => {
  app.use(path, (req, res, next) => { res.setHeader('Cache-Control', 'no-store'); next(); });
});
...
app.use('/api/clause-analysis', clauseAnalysisRouter);
app.use('/api/matter', matterRouter); // Google va Apple login
...
if (!process.env.VERCEL) {
  app.listen(PORT, () => {
    ...
    console.log('║                                              ║');
    console.log('║ XAVFSIZLIK (YANGI):                          ║');
    console.log(`║ TWO_FACTOR_KEY    : ${process.env.TWO_FACTOR_ENCRYPTION_KEY ? '✅ o\'rnatilgan (AES-256)      ' : '⚠️  yo\'q (2FA shifrlash zaif)    '}║`);
    console.log(`║ REFRESH_TOKEN_SEC : ${process.env.REFRESH_TOKEN_SECRET ? '✅ o\'rnatilgan              ' : '⚠️  yo\'q (JWT refresh zaif)      '}║`);
    console.log('║                                              ║');
    // Claude model

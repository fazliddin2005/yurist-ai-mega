...
async function loadB2BConversations(){
  try{
    ...
    // Chat bo'sh bo'lsa suggestion kartochkalarini ko'rsat
    const body=document.getElementById('b2bChatBody');
    if(body && !body.querySelector('.b2b-msg, .b2b-suggest-wrap')){
      b2bShowEmptyState();
    }
  }catch(e){ /* jim turamiz */ }
}
...
/* ---- B2B Suggestion kartochkalari (B2C dagi kabi) ---- */
const B2B_SUGGESTIONS = [
  {ic:'📝', bg:'#EEF2FF', fg:'#4F46E5', t:'Shartnoma',  d:'Xodim bilan mehnat shartnomasi tuzing'},
  {ic:'⚖️', bg:'#FEF3C7', fg:'#B45309', t:'Nizolar',    d:'Biznes nizolarni hal qilish tartibi qanday?'},
  {ic:'🏢', bg:'#ECFDF5', fg:'#059669', t:'Korporativ', d:"Kompaniya ustavi va nizomiga o'zgartirish kiritish"},
  {ic:'📋', bg:'#FDF4FF', fg:'#9333EA', t:'Majburiyat', d:'Ijarachi majburiyatlarini shartnomaga kiriting'},
  {ic:'🔒', bg:'#FFF7ED', fg:'#C2410C', t:'Maxfiylik',  d:'NDA (maxfiylik shartnomasi) namunasi kerak'},
  {ic:'📊', bg:'#F0F9FF', fg:'#0369A1', t:'Soliq',      d:"Xizmat ko'rsatish shartnomasi uchun QQS tartibi"},
];
const B2B_SUGGEST_I18N = {
  uz:[
    ['Shartnoma',"Xodim bilan mehnat shartnomasi tuzing"],
    ['Nizolar',"Biznes nizolarni hal qilish tartibi qanday?"],
    ['Korporativ',"Kompaniya ustavi va nizomiga o'zgartirish kiritish"],
    ['Majburiyat','Ijarachi majburiyatlarini shartnomaga kiriting'],
    ['Maxfiylik','NDA (maxfiylik shartnomasi) namunasi kerak'],
    ['Soliq',"Xizmat ko'rsatish shartnomasi uchun QQS tartibi"],
  ],
  ru:[
    ['Договор','Составьте трудовой договор с сотрудником'],
    ['Споры','Каков порядок разрешения бизнес-споров?'],
    ['Корпоратив','Внесение изменений в устав компании'],
    ['Обязательство','Включите обязательства арендатора в договор'],
    ['Конфиденц.','Нужен образец NDA (соглашения о конфиденциальности)'],
    ['Налоги','Порядок НДС для договора оказания услуг'],
  ],
  en:[
    ['Contract','Draft an employment contract with an employee'],
    ['Disputes','What is the procedure for resolving business disputes?'],
    ['Corporate','Amend the company charter and bylaws'],
    ['Obligation','Add tenant obligations to the lease agreement'],
    ['NDA','Need a template for a Non-Disclosure Agreement'],
    ['Tax','VAT procedure for a service agreement'],
  ],
};
function b2bRenderSuggest(){
  const c = document.getElementById('b2bSuggestGrid');
  if(!c) return;
  c.innerHTML = '';
  const lang = curLang || 'uz';
  const tr = B2B_SUGGEST_I18N[lang] || B2B_SUGGEST_I18N.uz;
  B2B_SUGGESTIONS.forEach(function(s,i){
    const title=(tr[i]&&tr[i][0])||s.t;
    const desc=(tr[i]&&tr[i][1])||s.d;
    const el=document.createElement('div');
    el.className='b2b-sg';
    el.innerHTML='<div class="b2b-sgi" style="background:'+s.bg+';color:'+s.fg+'">'+s.ic+'</div>'
      +'<div><div class="b2b-sgt">'+title+'</div><div class="b2b-sgd">'+desc+'</div></div>';
    el.onclick=function(){ b2bAskSuggest(desc); };
    c.appendChild(el);
  });
}
function b2bAskSuggest(q){
  const inp=document.getElementById('b2bChatInput');
  if(inp){ inp.value=q; b2bSendChat(); }
}
function b2bChatEmptyState(){
  const title = (window.I18N && window.I18N[curLang||'uz'] && window.I18N[curLang||'uz'].suggest_title) || "Nimani so'rashingiz mumkin";
  return '<div class="b2b-suggest-wrap">'
    +'<div class="b2b-suggest-title">✨ <span>'+title+'</span></div>'
    +'<div class="b2b-suggest-grid" id="b2bSuggestGrid"></div>'
    +'</div>';
}
function b2bShowEmptyState(){
  const body=document.getElementById('b2bChatBody');
  if(body){ body.innerHTML=b2bChatEmptyState(); b2bRenderSuggest(); }
}
...
async function openB2BConversation(id){
  ...
  try{
    ...
    const body=document.getElementById('b2bChatBody');
    if(!conversation.messages.length){ b2bShowEmptyState(); }
...
async function b2bHandleChatFile(input) {
  ...
  userMsg.className = 'b2b-msg user';
  userMsg.innerHTML = `<div class="b2b-msg-bub">📎 <b>${file.name}</b> — tahlil so'raldi</div>`;
  body.appendChild(userMsg);
  body.scrollTop = body.scrollHeight;

  // Typing animatsiya
  const typing = document.createElement('div');
  typing.className = 'b2b-msg';
  typing.innerHTML = `<div class="b2b-msg-av">🤖</div><div class="b2b-msg-bub"><div class="typing-dots"><span></span><span></span><span></span></div><div style="font-size:12px;color:#94a3b8;margin-top:6px">Hujjat o'qilmoqda...</div></div>`;
  ...
  try {
    ...
    const resp = await apiPost(`/b2b/chat/${currentWorkspace.id}/file`, {
      fileName: file.name,
      fileBase64: base64,
    ...
    aiMsg.className = 'b2b-msg';
    const formatted = typeof formatChatReply === 'function'
      ? formatChatReply(resp.reply || 'Tahlil bajarildi.')
      : (resp.reply || 'Tahlil bajarildi.');
    aiMsg.innerHTML = `<div class="b2b-msg-av">🤖</div><div class="b2b-msg-bub">${formatted}</div>`;
    body.appendChild(aiMsg);

    // Chat tarixiga qo'shish
    b2bChatHistory.push(
      { role: 'user', content: `📎 ${file.name} faylini tahlil qiling` },
      { role: 'assistant', content: resp.reply || '' }
    );
    // Kredit UI yangilash
    if (typeof resp.creditsLeft === 'number') {
      credits = resp.creditsLeft;
    ...
    errMsg.className = 'b2b-msg';
    errMsg.innerHTML = `<div class="b2b-msg-av">🤖</div><div class="b2b-msg-bub" style="color:#ef4444">Hujjatni o'qishda xatolik: ${e.message || "Noma'lum xato"}.<br>Iltimos <b>.pdf, .docx yoki .txt</b> formatidagi fayl yuboring.</div>`;
